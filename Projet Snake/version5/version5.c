...
void drawSnake(int x[], int y[], char currentDirection);
void progress(int x[], int y[], char direction, bool * adrIsColliding, bool * adrHasEatApple);
void updateSnake(int x[], int y[], bool * adrHasEatApple, int * adrNbAppleEated);

//Procédures liés à l'Input
char getInput();
void defDirection(char * currentDirection, char currentInput);
void exitSnake(bool * adrIsWorking, char currentInput, bool isColliding);
...
int currentAppleX[500]; //Coordonnée X de l'actuelle pomme
int currentAppleY[500]; //Coordonnée Y de l'actuelle pomme
...
int nbAppleToWin = NB_APPLE_TO_WIN;

int nbBlock = NB_BLOCK;
...
int main(){
    ...
    while (isGameWorking == true){
        ...
        addApple();
        drawSnake(snakeX, snakeY, direction); //Dessine le serpent une première fois aux coordonnées de départ
        nbAppleEated = 0;

        while (nbAppleEated < nbAppleToWin && isGameWorking == true){
            ...
            drawSnake(snakeX, snakeY, direction); //Affiche le serpent aux coordonnées rentrés en paramètres

            exitSnake(&isGameWorking, currentInput, isSnakeColliding);
            updateSnake(snakeX, snakeY, &hasSnakeEatApple, &nbAppleEated); //Met à jour les infos liés au serpent : sa vitesse, son nombre de pomme mangé et sa taille
        }
    ...
    for (int i = 0; i < nbBlock; i++){
        ...
        for (int i = 0; i < BLOCK_SIZE; i++){
            ...
            for (int j = 0; j < BLOCK_SIZE; j++){
                gameMap[currentBlockY + i][currentBlockX + j] = BLOCK_CHAR;
            }
        }
    }
}
...
void addApple(){
    ...
    for (int nbAppleCreated = 0; nbAppleCreated < nbAppleToWin; nbAppleCreated++){
        do{
                

            currentAppleX[nbAppleCreated] = (rand() % ((MAP_LIMIT_X_MAX) - MIN_POS_APPLE)) + MIN_POS_APPLE;
            currentAppleY[nbAppleCreated] = (rand() % ((MAP_LIMIT_Y_MAX) - MIN_POS_APPLE)) + MIN_POS_APPLE;

            for (int i = 0; i < currentSnakeLength; i++){
                if ((currentAppleX[nbAppleCreated] == snakeX[i]) && (currentAppleY[nbAppleCreated] == snakeY[i])){
                    isGood = false;
                }
            }

        }while(gameMap[currentAppleY[nbAppleCreated]][currentAppleX[nbAppleCreated]] == BLOCK_CHAR || isGood != true);

            
        displayChar(currentAppleX[nbAppleCreated], currentAppleY[nbAppleCreated], APPLE_CHAR);
    }
}
...
*
*/
void drawSnake(int snakeX[], int snakeY[], char currentDirection){
    ...
    if (currentDirection == RIGHT){
        displayChar(snakeX[0], snakeY[0], SNAKE_HEAD_RIGHT);
    }

    else if (currentDirection == LEFT){
        displayChar(snakeX[0], snakeY[0], SNAKE_HEAD_LEFT);
    }

    else if (currentDirection == UP){
        displayChar(snakeX[0], snakeY[0], SNAKE_HEAD_UP);
    }

    else if (currentDirection == DOWN){
        displayChar(snakeX[0], snakeY[0], SNAKE_HEAD_DOWN);
    }
}
...
void progress(int snakeX[], int snakeY[], char direction, bool * adrIsColliding, bool * adrHasEatApple){
    ...
    if (gameMap[snakeY[0]][snakeX[0]] == BLOCK_CHAR){
        *adrIsColliding = true;
    }
    ...
    for (int i = 0; i < nbAppleToWin - 1; i++){

        if ((snakeX[0] == currentAppleX[i]) && (snakeY[0] == currentAppleY[i])){
            *adrHasEatApple = true;
        }
    }
...
void exitSnake(bool * adrIsWorking, char currentInput, bool isColliding){
    ...
    if (currentInput == STOP_CHAR || isColliding == true){
        *adrIsWorking = false;
    }
}
