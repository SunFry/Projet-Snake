...
* @date 2024-11-17
*
* Ce programme est une troisième version d'un jeu snake dans le terminal Linux. Cette troisième version consiste au
* déplacement du serpent dans un plateau définis avec des bordures et des pavés placés aléatoirement sur le plateau. 
* Le programme démarre sur le serpent se dirigeant vers la droite.
* L'utilisateur peut ensuite le diriger, si il rentre en collision avec un élément d'un pavé ou de la bordure, celà mettra fin au programme
...
void progress(int x[], int y[], char direction, int * adrIsColliding);

//Procédures liés à l'Input
char getInput();
void defDirection(char * currentDirection, char currentInput);
void exitSnake(int * adrIsWorking, char currentInput, int adrIsColliding);
...
int main(){
    ...
    while (isWorking == 1){
        ...
        progress(snakeX, snakeY, direction, &isColliding); //Déplace les coordonnées dans la direction rentré en paramètre
        drawSnake(snakeX, snakeY); //Affiche le serpent aux coordonnées rentrés en paramètres

        exitSnake(&isWorking, currentInput, isColliding);
    }
...
void initMap(){
    ...
    for (int x = 1; x < MAP_LIMIT_X_MAX; x++){  //Initialise la bordure haute
        gameMap[1][x] = WALL_CHAR; 
    }

    for (int y = 2; y < MAP_LIMIT_Y_MAX - 1; y++){ //Initialise chaque ligne intérieur du tableau contenant un caractère # en début et fin de ligne 
                                                   //et avec un nombre précis d'espace à l'intérieur
        gameMap[y][1] = WALL_CHAR;

        for (int x = 2; x < MAP_LIMIT_X_MAX - 1; x++){
    ...
    for (int x = 1; x < MAP_LIMIT_X_MAX; x++){ //Initialise la bordure basse
...
void drawMap(){
    ...
    for (int y = 1; y < MAP_LIMIT_Y_MAX; y++){

        for (int x = 1; x < MAP_LIMIT_X_MAX; x++){
...
void drawSnake(int snakeX[], int snakeY[]){
    ...
    for (int i = 1; i < SNAKE_LENGTH; i++){ /*! boucle parcourant les éléments du serpent */
...
*
*/
void progress(int snakeX[], int snakeY[], char direction, int * adrIsColliding){

    /* Gestion du déplacement du serpent */

    //1.
    int newCoordX[SNAKE_LENGTH];
    int newCoordY[SNAKE_LENGTH];

    if ( gameMap[snakeY[SNAKE_LENGTH - 1]][snakeX[SNAKE_LENGTH - 1]] != WALL_CHAR ){

        eraseChar(snakeX[SNAKE_LENGTH - 1], snakeY[SNAKE_LENGTH - 1]); //2.
    }
...
void exitSnake(int * adrIsWorking, char currentInput, int adrIsColliding){
    ...
    if (currentInput == STOP_CHAR || adrIsColliding == 1){
        *adrIsWorking = 0;
    }
}
